    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {products.map((product) => (
          <Link href={`/product/${product._id}`} className="block w-80">
          <Card
            key={product._id}
          ...
          >
            ...
            <CardContent className="p-4">
              ...
              <p className="mt-4 font-bold text-lg text-right text-primary">{formatCurrency(product.price)}</p>
            </CardContent>
          </Card>
          </Link>
        ))}
      ...
      <div className="mt-8 flex justify-center">
        <Button onClick={() => window.location.reload()} className="text-white">Tải lại</Button>
      </div>
    </div>
  );
}
